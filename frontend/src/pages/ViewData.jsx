...
import { Table, Form, Dropdown, Button, Col, Row, Pagination } from 'react-bootstrap';
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (handleFilterChangeReturn(filters)) {
          setDataCount(response.data.length / 10)
        }
        else {
          fetchDataCount();
        }
      ...
      <Row className="mb-3">
        ...
        <Col xs={12} sm={6} md={4} lg={2} className="mb-2">
          ...
          <Dropdown show={showFilters.rowsPerPage} className="mt-2">
            <Dropdown.Menu style={{ minWidth: '200px' }}>
            <Dropdown.Item onClick={() => handleRowsPerPageSelect(20)}>10</Dropdown.Item>
